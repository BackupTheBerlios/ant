...
value rec encode_node (node : Evaluate.node_type) = match node with
[ `Nodes ns                    -> encode_node_list ns
| `Command loc cmd             -> make_node1 sym_Command loc
                                    (encode_env_cmd "<unnamed>" cmd)
| `CommandBox loc c            -> make_node1 sym_CommandBox loc
                                    (encode_box_cmd c)
| `GfxCommand loc c            -> make_node1 sym_GfxCommand loc
                                    (encode_gfx_cmd c)
| `NewGalley loc n m           -> make_node2 sym_NewGalley loc
                                    (Machine.uc_string_to_char_list n)
                                    (encode_skip_arg m)
| `NewLayout loc n w h         -> make_node3 sym_NewLayout loc
                                    (Machine.uc_string_to_char_list n)
                                    (encode_skip_arg w)
                                    (encode_skip_arg h)
| `NewArea loc n x y w h t b c -> make_node8 sym_NewArea loc
                                    (Machine.uc_string_to_char_list n)
                                    (encode_skip_arg x)
                                    (encode_skip_arg y)
                                    (encode_skip_arg w)
...
| `ShipOut loc e o n           -> make_node3 sym_ShipOut loc
                                    (Machine.uc_string_to_char_list e)
                                    (Machine.uc_string_to_char_list o)
                                    (Types.Number (num_of_int n))
| `AddToGalley loc g n         -> make_node2 sym_AddToGalley loc
                                    (Machine.uc_string_to_char_list g)
                                    (encode_node_list n)
| `PutGalleyInVBox loc a n     -> make_node2 sym_PutGalleyInVBox loc
                                    (Types.Bool a)
                                    (Machine.uc_string_to_char_list n)
| `ModifyGalleyGlue loc f      -> make_node1 sym_ModifyGalleyGlue loc
                                    (encode_glue_function f)
| `Paragraph loc b             -> make_node1 sym_Paragraph loc (encode_node_list b)
| `BeginGroup loc              -> make_node0 sym_BeginGroup loc
| `EndGroup loc                -> make_node0 sym_EndGroup loc
| `Float loc n b               -> make_node2 sym_Float loc
                                    (Machine.uc_string_to_char_list n)
                                    (encode_node_list b)
| `Glyph loc g                 -> make_node1 sym_Glyph loc (Types.Number (num_of_int g))
| `Letter loc char             -> make_node1 sym_Letter loc (Types.Char char)
| `Space loc                   -> make_node0 sym_Space loc
| `Glue loc w h i d            -> make_node4 sym_Glue loc
                                    (encode_dim_arg w)
                                    (encode_dim_arg h)
                                    (Types.Bool i)
                                    (Types.Bool d)
| `Break loc p h pre post no   -> make_node5 sym_Break loc
                                    (match p with
                                    ...
                                    (encode_node_list no)
| `Rule loc w h d              -> make_node3 sym_Rule loc
                                    (encode_dim_arg w)
                                    (encode_dim_arg h)
                                    (encode_dim_arg d)
| `Image loc f w h             -> make_node3 sym_Image loc
                                    (Machine.uc_list_to_char_list (UString.of_string f))
                                    (encode_skip_arg w)
                                    (encode_skip_arg h)
| `Accent loc a c              -> make_node2 sym_Accent loc
                                    (Types.Char a)
                                    (encode_node_list c)
| `HBox loc b                  -> make_node1 sym_HBox loc
                                    (encode_node_list b)
| `HBoxTo loc w b              -> make_node2 sym_HBoxTo loc
                                    (encode_skip_arg w)
                                    (encode_node_list b)
| `HBoxSpread loc a b          -> make_node2 sym_HBoxSpread loc
                                    (encode_skip_arg a)
                                    (encode_node_list b)
| `VBox loc b                  -> make_node1 sym_VBox loc
                                    (encode_node_list b)
| `VBoxTo loc h b              -> make_node2 sym_VBoxTo loc
                                    (encode_skip_arg h)
                                    (encode_node_list b)
| `VBoxSpread loc a b          -> make_node2 sym_VBoxSpread loc
                                    (encode_skip_arg a)
                                    (encode_node_list b)
| `Phantom loc h v n           -> make_node3 sym_Phantom loc
                                    (Types.Bool h)
                                    (Types.Bool v)
                                    (encode_node_list n)
| `HLeaders loc w n            -> make_node2 sym_HLeaders loc
                                    (encode_dim_arg w)
                                    (encode_node_list n)
| `VInsert loc b ns            -> make_node2 sym_VInsert loc
                                    (Types.Bool b)
                                    (encode_node_list ns)
| `Table loc n                 -> make_node1 sym_Table loc
                                    (encode_node_list n)
| `TableEntry loc l r t bl b c -> make_node6 sym_TableEntry loc
                                    ...
                                    (encode_node_list c)
| `Math loc n                  -> make_node1 sym_Math loc
                                    (encode_node_list n)
| `MathCode loc c n            -> make_node2 sym_MathCode loc
                                    (encode_math_code c)
                                    (encode_node_list n)
| `MathChar loc (cd, (f1,f2), (c1,c2)) ->
    make_node1 sym_MathChar loc
      (Types.Tuple
        [|ref (encode_math_code cd);
          ...
          ref (Types.Char c2)|])
| `SubScript loc n             -> make_node1 sym_SubScript loc (encode_node_list n)
| `SuperScript loc n           -> make_node1 sym_SuperScript loc (encode_node_list n)
| `Fraction loc n d l r t      -> make_node5 sym_Fraction loc
                                    (encode_node_list n)
                                    (encode_node_list d)
                                    (encode_node l)
                                    (encode_node r)
                                    (encode_skip_arg t)
| `Underline loc n             -> make_node1 sym_Underline loc (encode_node_list n)
| `Overline loc n              -> make_node1 sym_Overline loc (encode_node_list n)
| `MathAccent loc f c n        -> make_node3 sym_MathAccent loc
                                    (Types.Number (num_of_int f))
                                    (Types.Char c)
                                    (encode_node_list n)
| `Root loc sf sc lf lc n      -> make_node5 sym_Root loc
                                    (Types.Number (num_of_int sf))
                                    (Types.Char sc)
                                    (Types.Number (num_of_int lf))
                                    (Types.Char lc)
                                    (encode_node_list n)
| `LeftRight loc l n r         -> make_node3 sym_LeftRight loc
                                    (encode_node_list l)
                                    (encode_node_list n)
                                    (encode_node_list r)
| `MathStyle loc s             -> make_node1 sym_MathStyle loc
                                    (encode_math_style s)
| `IndexPosition loc p         -> make_node1 sym_IndexPosition loc
                                    (encode_index_position p)
]
...
value rec decode_node name node = do
{
  ...
  match !node with
  [ Types.Nil      -> `Nodes []
  | Types.List _ _ -> `Nodes (decode_node_list name node)
  | Types.Tuple xs -> do
    {
        ...
        [ Types.Symbol s -> do
          {
            if s = sym_Command then
              ...
              `Command loc (decode_env_cmd name c)
            else if s = sym_CommandBox then
              let (loc, c) = decode_tuple1 name xs in
              `CommandBox loc (decode_box_cmd name c)
              ...
              `Glyph loc (decode_int name g)
            else if s = sym_Letter then
              let (loc, c) = decode_tuple1 name xs in
              `Letter loc (decode_char name c)
            ...
            else if s = sym_MathChar then do
            {
              ...
              let (loc, l, n, r) = decode_tuple3 name xs in
              `LeftRight loc
                (decode_node_list name l)
                (decode_node_list name n)
                (decode_node_list name r)
